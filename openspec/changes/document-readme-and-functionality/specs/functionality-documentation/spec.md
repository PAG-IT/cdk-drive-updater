...
### Requirement: FUNCTIONALITY.md documents non-default build and release packaging behavior

FUNCTIONALITY.md SHALL include a build/release packaging section when the repository has non-default build scripts, executable resources, release profile settings, or generated release artifact rules.

#### Scenario: AI agent preserves the slim release artifact

- **WHEN** an AI agent reads the Build And Release Packaging section in FUNCTIONALITY.md
- **THEN** it SHALL know that `build.rs` embeds `resources/logo.ico`, Cargo release profile settings reduce executable size, and `scripts/build-release.ps1` publishes only `dist/cdk-drive-updater.exe`
